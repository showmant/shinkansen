...
/**
 * VOICEVOX に渡す読み。
 * ひらがなだけの文だと単語の中の「は」まで助詞とみなされて「ワ」と読まれてしまう
 * (よこはません → ヨコワマセン)。単独で置かれた助詞の「は」以外はカタカナにして読みを固定する。
 * 直すのは合成に投げる文字列だけで、画面に出す文は変えない。
 */
const readingOf = (text) => text.replace(/は/g, (ha, at) =>
  text[at - 1] === " " && text[at + 1] === " " ? ha : "ハ");
